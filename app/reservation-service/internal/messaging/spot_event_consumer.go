...
func (c *kafkaSpotEventConsumer) Start(ctx context.Context, handler SpotEventHandler) error {
	for {
		...
		if err := json.Unmarshal(message.Value, &event); err != nil {
			log.Printf("failed to decode reservation-service spot event: %v", err)
			if commitErr := c.reader.CommitMessages(ctx, message); commitErr != nil {
				return fmt.Errorf("failed to commit malformed reservation-service spot kafka message: %w", commitErr)
			}
			continue
		}

		if err := handler.HandleSpotEvent(ctx, event); err != nil {
			log.Printf("failed to handle reservation-service spot event %s: %v", event.EventID, err)
			select {
			case <-ctx.Done():
				return nil
			case <-time.After(2 * time.Second):
		...
		if err := c.reader.CommitMessages(ctx, message); err != nil {
			return fmt.Errorf("failed to commit reservation-service spot kafka message: %w", err)
		}
	}
}
